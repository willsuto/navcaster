...
exports.forecastData = exports.currentWeather = void 0;
function generateWeatherGrid(timeOffset = 0) {
    const grid = [];
    ...
    for (let lat = 20.0; lat <= 32.0; lat += 2.0) {
        for (let lng = -98.0; lng <= -78.0; lng += 2.0) {
            ...
            const distFromStorm = Math.sqrt(Math.pow(lat - stormLat, 2) + Math.pow(lng - stormLng, 2));
...
exports.currentWeather = generateWeatherGrid(0);
// Generate 24-hour forecast in 6-hour increments
exports.forecastData = [0, 6, 12, 18, 24].map((hours) => {
