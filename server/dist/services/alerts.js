...
function generateAlerts(vessels, currentWeather, forecast, currentRiskScores) {
    const alerts = [];
    for (const vessel of vessels) {
        ...
        for (const forecastPoint of forecast) {
            const futureRisk = (0, riskScore_1.calculateRiskScore)(vessel, forecastPoint.weatherGrid);
            const hoursAhead = Math.round((new Date(forecastPoint.timestamp).getTime() - Date.now()) / (1000 * 60 * 60));
            if (hoursAhead <= 0)
                continue;
